...
from word import _idn_by_word_or_number
...
class WordMoreTests(WordTests):
    ...
    def test_verb_paren_object(self):
        ...
        with self.assertNewWord():
            blob = oobleck(some_object)   # FIXME:  a getter, not a setter
    ...
    def test_verb_paren_object_deferred_subject(self):
        ...
        That's a verb such as subject.verb that is generated by an attribute.
...
class WordUtilities(WordTests):
    ...
    def test_idn_by_word_or_number(self):
        agent = self.lex.agent
        self.assertEqual(qiki.Word._ID_AGENT, _idn_by_word_or_number(agent.idn))
        self.assertEqual(qiki.Word._ID_AGENT, _idn_by_word_or_number(agent))
        with self.assertRaises(TypeError):
            _idn_by_word_or_number('')
        with self.assertRaises(TypeError):
            _idn_by_word_or_number(0)
